...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            return scheme_eval(procedure.apply_macro(rest, env), env, True)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Different amount of formals then values")
        while formals != nil:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Wrong number of items")
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    while expressions.second != nil and scheme_truep(scheme_eval(expressions.first, env)):
        expressions = expressions.second
    return scheme_eval(expressions.first, env, True)
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    while expressions.second != nil and scheme_falsep(scheme_eval(expressions.first, env)):
        expressions = expressions.second
    return scheme_eval(expressions.first, env, True)
    # END PROBLEM 13
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            elif clause.second is nil:
                return test
            else:
                return scheme_truep(eval_all(clause.first, env))
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        symbol = Pair(binding.first, symbol)
        expr = Pair(scheme_eval(binding.second.first, env), expr)
        bindings = bindings.second
    check_formals(symbol)
    return env.make_child_frame(symbol, expr)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        do_define_form = MacroProcedure(target.second, expressions.second, env)
        env.define(target.first, do_define_form)
        return target.first
        # END PROBLEM 10
    else:
        raise SchemeError("incorrect format")
    # END Problem 21
